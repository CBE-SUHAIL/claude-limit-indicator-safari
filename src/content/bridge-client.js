(() => {
	...
	function getRuntime() {
		// Safari uses browser.runtime; Chrome uses chrome.runtime
	...
	function injectBridgeOnce() {
		...
		if (!runtime) {
			console.warn('[ClaudeCounter] No extension runtime found');
		...
		bridgeReadyPromise = new Promise((resolve) => {
			...
			// runtime.getURL works in both Safari (browser.runtime) and Chrome (chrome.runtime)
			script.src = runtime.getURL('src/injected/bridge.js');
			script.onload = () => resolve(true);
			script.onerror = (e) => {
				console.error('[ClaudeCounter] Failed to inject bridge.js:', e);
				resolve(false);
			};
